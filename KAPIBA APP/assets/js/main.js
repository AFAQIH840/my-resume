...
(function() {
   ...
   */
  window.addEventListener('load', () => {
    let flashcardContainer = select('.flashcard-container');
    if (flashcardContainer) {
      let flashcardIsotope = new Isotope(flashcardContainer, {
        itemSelector: '.flashcard-item'
      });

      let flashcardFilters = select('#flashcard-flters li', true);

      on('click', '#flashcard-flters li', function(e) {
        e.preventDefault();
        flashcardFilters.forEach(function(el) {
          el.classList.remove('filter-active');
        });
        this.classList.add('filter-active');

        flashcardIsotope.arrange({
          filter: this.getAttribute('data-filter')
        });
        flashcardIsotope.on('arrangeComplete', function() {
          AOS.refresh()
        });
      }, true);
    }
  ...
  /**
   * flashcard details slider
   */
  new Swiper('.flashcard-details-slider', {
    speed: 400,
    loop: true,
    autoplay: {
      delay: 5000,
